...
class IncrDataTableScan extends DataTableScan implements IncrTableScan {
  ...
  public CloseableIterable<FileScanTask> planFiles() {
    ...
    // Use parent's planFiles API to return incremental files when planning
    return super.planFiles(ops, snapshot, filter(), isCaseSensitive(), colStats);
  }

  private Snapshot snapshot(long id) {
    Snapshot snapshot = table.snapshot(id);
    Preconditions.checkState(snapshot != null, "Cannot find snapshot corresponding to id %s", id);
    return snapshot;
  }
